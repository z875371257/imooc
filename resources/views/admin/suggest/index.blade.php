...
@section('title', '意见反馈管理')

@section('content')
    <div class="mws-panel grid_8">

        <div class="mws-panel-header">
            <span><i class="icon-table"></i>意见反馈列表页</span>
        </div>
        <!-- 表单提交后发生错误信息的显示 -->
        ...
        <div class="mws-panel-body no-padding" >
            <div role="grid" class="dataTables_wrapper" id="DataTables_Table_1_wrapper">
                <form action="{{url('admin/suggest')}}" method='get'>
                ...
                <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1"
                    ...
                    <thead>
                    <tr role="row">
                        <th  role="columnheader" rowspan="1" colspan="1" style="width: 30px;" aria-label="Rendering engine: activate to sort column descending">
                            ID
                        </th>
                        <th  role="columnheader" rowspan="1" colspan="1" style="width: 150px;" aria-label="Browser: activate to sort column ascending">
                            建议类型
                        </th>
                        <th  role="columnheader" rowspan="1" colspan="1" style="width: 400px;" aria-label="Platform(s): activate to sort column ascending">
                            内容
                        </th>
                        ...
                        <th  role="columnheader" rowspan="1" colspan="1" style="width:100px;" aria-label="Engine version: activate to sort column ascending">
                            状态
                        </th>

                        <th  role="columnheader"  rowspan="1" colspan="1" style="width: 200px;" aria-label="CSS grade: activate to sort column ascending">
                            操作
                        </th>
                    </tr>
                    </thead>
                    ...
                    @foreach($res as $k=>$v)
                        <tr class=" ">
                            ...
                            <td class=" " >
                                {{--{{$v->status}}--}}
                                @if($v->status == 0)  未处理 @else  已处理 @endif
                            </td>
                            <td class=" ">
                                <a href="/admin/suggest/{{$v->id}}/edit/" class='btn btn-success'>处理</a>
                ...
                <script>
                    ...
                    function delUser(id)
                    {
                        ...
                        layer.confirm('确定删除？', {
                            ...
                            $.post("{{url('admin/suggest')}}/"+id,{'_method':'delete','_token':'{{csrf_token()}}'},function(data){
                                if(data.status == 0){
